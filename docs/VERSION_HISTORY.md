...
| Sep 2025 | v0.7 | **Resolved 2026-08-20 — not a rename at all.** Per the author: Helm=Executive, Shield=Army, Stewards=Property/Landowners (and, completing the pattern already given in `CAMS-CAN-MASTER-REFERENCE.md`'s own node table: Lore=Knowledge, Archive=Memory, Craft=Trades, Hands=Labor, Flow=Commerce). These were **two parallel label sets for the same eight nodes from the start** — a mnemonic/public-facing set (Helm/Shield/Lore/...) and a formal-sociological-category set (Executive/Army/Knowledge/...) — not a canonical name that was later reverted. `CAMS-CAN-MASTER-REFERENCE.md`'s "terminology standardized (eliminated inconsistencies with 'Helm/Executive')" describes tidying up *inconsistent use of the pairing*, not choosing one label over the other. **The mnemonic set (Helm/Shield/Lore/...) became the dominant public-facing naming going forward** because the author judged it more universal and less Western-institution-specific than the sociological-category set — "Executive" and "Army" carry modern-state connotations that "Helm" and "Shield" don't, which matters for a framework claiming applicability across very different civilisational contexts. This same month, separately, a thermodynamic/entropy formalization was validated against Norway, Singapore, USA, and China data, and the deliberative-vs-reactive governance-mode distinction was formalised. | — |
| Nov 2025 | v0.8 / RD-001 | **Resolved 2026-08-20 — not actually a conflict.** Per the author: stress was never negative *as a construct* — it was recorded as a negative integer purely as a calculation convenience, so that `V = C + K + S + 0.5A` with signed S gave the same result as `V = C + K − S + 0.5A` with unsigned S, without needing an explicit subtraction. RD-001 standardised this to the current unsigned (1–10) encoding. `CAMS-CAN-MASTER-REFERENCE.md`'s "eliminated signed encoding" and `cams-project-history`'s "clarified what negative values mean" descriptions are two framings of the same encoding change, not competing accounts of what stress means. Metrics (Coherence, Capacity, Stress, Abstraction) were originally scored **0–5**, later widened to the current **1–10** scale. **This transition's date is closed as genuinely unresolved** — see "Items Flagged," below, for the full archival-search finding and the separate open question of what conversion mapping was used. | Signed stress encoding (calculation convenience only); 0–5 metric scale |
...
3. ~~The date and rationale of the second node-naming change~~ — **resolved
   2026-08-20: there was no rename.** Helm/Executive, Shield/Army,
   Lore/Knowledge, Archive/Memory, Stewards/Property, Craft/Trades,
   Hands/Labor, and Flow/Commerce are confirmed by the author as two
   parallel label sets for the same eight nodes from the start — not
   sequential canonical states. The mnemonic set became the dominant
   public-facing naming because the author judged it more universal and
   less tied to modern-Western institutional vocabulary than the
   sociological-category set. See table above.
4. ~~A dated decision record for RD-003~~ — **closed 2026-08-20, unresolved:
   the author does not recall what RD-003 specifically was.** Not
   recoverable; not expected to be dated further. The bond-formula
   retirement it's cited for in code comments (`juno_backcalc.py`) is
   otherwise well-documented (see the ~Jun 2026 CAMS v2.4 row above) —
   only the RD-003 label itself is unidentified.
...
9. **Scale transition (0–5 → 1–10): date unknown — closed as genuinely
   unresolved, 2026-08-20.** A dedicated archive search (author-conducted)
   found no scale-change record, methodology note, version log, or dated
   statement — only unrelated text. Per the author's own methodological
   judgement, this should be treated as an unknown metadata field rather
   than inferred from later spreadsheets, and any future estimate must be
   labelled provisional and distinguished from the date of the earliest
   surviving 1–10 dataset. It would take one of: a dated master dataset
   using 1–10 values, an earlier counterpart demonstrably using 0–5, a
   methodology draft/email/notebook/revision-history describing the
   recoding, or file metadata corroborated by content — none of which has
   surfaced. **Separately flagged as substantive, not just archival:** the
   *mapping* used for this conversion (linear rescale vs. re-scored from
   scratch, and how thresholds/rounding were handled) has not been
   documented either, and matters for whether pre- and post-transition data
   are directly comparable — a scale change of this kind is not cosmetic
   unless that mapping is specified.
